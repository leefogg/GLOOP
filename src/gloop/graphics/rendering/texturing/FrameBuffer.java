...
public class FrameBuffer implements Disposable {
	...
	private void addColorAttachments(FrameBufferColorTexture[] attachments) {
		Collections.addAll(colorAttachments, attachments);
	}
	...
	private static void setCurrent(FrameBuffer buffer) {
	...
	private static PixelFormat[] createFormatList(int count, PixelFormat format) {
	...
	public static FrameBuffer getCurrent() { return FrameBufferManager.getCurrentFrameBuffer(); }
	...
	public static BufferedImage read(int startX, int startY, int width, int height) {
	...
	// Reads the currently bound framebuffer's pixel's state
	// NOTE: Ensure that all required geometry has been drawn (if nessersary) with glFlush and glFinish
	public static Vector4f read(int x, int y) {
